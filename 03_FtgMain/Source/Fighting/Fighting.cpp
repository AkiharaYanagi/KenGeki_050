...
//		m_mutualChara->SetpFtgGrp ( m_pFtgGrp );
	...
	//ラウンド取得
	void Fighting::AddRound_1p () { m_round->AddRound_1p (); }
	void Fighting::AddRound_2p () { m_round->AddRound_2p (); }

	//試合(マッチ)終了判定
	bool Fighting::IsEndMutch () { return m_round->IsEndMutch (); }

	//終了
	bool Fighting::IsEnd () const { return m_demoActor->IsEnd (); }
	bool Fighting::IsDrawEnd () const { return m_demoActor->IsDrawEnd (); }
