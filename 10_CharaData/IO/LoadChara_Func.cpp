...
//
//=================================================================================================
#include "10_CharaData/LoadChara_Func.h"
...
namespace CHARA
{
	...
	void LoadChara_Func::LoadChara ( CUPR_BYTE buf, UINT32 & pos, Chara & ch )
	{
		LoadCharaScript ( buf, pos, ch );
		LoadCharaImage ( buf, pos, ch );
	}

	void LoadChara_Func::LoadCharaScript ( CUPR_BYTE buf, UINT32 & pos, Chara & ch )
	{
...
//		LoadGarnish ( buf, pos, ch );	//Garnish
	...
	void LoadChara_Func::LoadCharaImage ( CUPR_BYTE buf, UINT32 & pos, Chara & ch )
	{
	...
	void LoadChara_Func::LoadCompend ( CUPR_BYTE buf, UINT32 & pos, Compend & cmpd )
	{
		//@afford メモリコントローラ
...
//		ch.AddpAction ( aryAct, nAct );
	...
	//フレーム
	void LoadChara_Func::LoadFrame ( CUPR_BYTE buf, UINT32 & pos, Frame & frm )
	{
		//グループ (Editorで用いる値)
...
//		m_utl.LoadAryInt ( buf, pos, frm.GetaVersatile () );
	...
	//スクリプト・戦闘パラメータ
	void LoadChara_Func::LoadFrmPrm_Btl ( CUPR_BYTE buf, UINT32 & pos, Frame & frm )
	{
		//設定用一時変数
	...
	//スクリプト・演出パラメータ
	void LoadChara_Func::LoadFrmPrm_Stg ( CUPR_BYTE buf, UINT32 & pos, Frame & frm )
	{
		//設定用一時変数
	...
	void LoadChara_Func::LoadCommand ( CUPR_BYTE buf, UINT32 & pos, Chara & ch )
	{
		//コマンド個数 と メモリの確保
...
//				bool bNot = (bool)buf [ pos ++ ];
	...
	void LoadChara_Func::LoadBranch ( CUPR_BYTE buf, UINT32 & pos, Chara & ch )
	{
		//ブランチ個数 と メモリの確保
...
//		std::unique_ptr < P_Brc [] > aryBrc = std::make_unique < P_Brc [] > ( nBrc );
	...
	void LoadChara_Func::LoadRoute ( CUPR_BYTE buf, UINT32 & pos, Chara & ch )
	{
		//ルート個数 と メモリの確保
...
//		UP_AP_Route aryRut = std::make_unique < P_Rut [] > ( nRut );
	...
	void LoadChara_Func::LoadListName ( CUPR_BYTE buf, UINT32 & pos, A_STR & aStr )
	{
		//個数
